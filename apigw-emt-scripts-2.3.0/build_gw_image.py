...
def _validateGeneralArgs():
    ...
    if (args.apiManager, args.factoryFed, args.fed, args.pol) == (False, False, None, None):
        fail("Must specify API Manager, FED, POL or factory FED.")
    if args.apiManager:
        if (args.factoryFed, args.fed, args.pol, args.env, args.fedPassFile) != (False, None, None, None, None):
            fail("API Manager cannot be specified in combination with FED, POL, ENV or FED passphrase.")
    if args.factoryFed:
        if (args.fed, args.pol, args.env, args.fedPassFile) != (None, None, None, None):
            fail("Factory FED cannot be specified in combination with FED, POL, ENV or FED passphrase.")
